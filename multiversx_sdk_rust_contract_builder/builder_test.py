...
input_directory = Path("./testdata/input")
expected_directory = Path("./testdata/expected")
output_directory = Path("./testdata/output")


def test_build_project_adder():
    outcome = builder.build_project(
        project_path=input_directory / "adder",
    ...
    expected_src_package = PackagedSourceCode.from_file(expected_directory / "adder-1.2.3.source.json")
    ...
    assert actual_wat == (expected_directory / "adder.wat").read_text()


def test_build_project_empty():
    outcome = builder.build_project(
        project_path=input_directory / "empty",
        parent_output_directory=output_directory,
        specific_contract=None,
        cargo_target_dir=Path("/tmp/cargo-target-dir"),
        no_wasm_opt=False,
    ...
    expected_src_package = PackagedSourceCode.from_file(expected_directory / "empty-4.5.6.source.json")
    ...
    assert_equal_src_package(actual_src_package, expected_src_package)
    assert actual_wat == (expected_directory / "empty.wat").read_text()
